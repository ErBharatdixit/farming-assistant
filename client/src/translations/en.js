// English translations
export const en = {
      // Common
      appName: "FarmAssist",
      tagline: "Farming Made Smarter",

      // Greetings
      goodMorning: "Good Morning",
      goodAfternoon: "Good Afternoon",
      goodEvening: "Good Evening",
      goodNight: "Good Night",

      // Navigation
      dashboard: "Dashboard",
      crops: "Crops",
      weather: "Weather",
      logout: "Logout",
      login: "Login",
      register: "Register",
      getStarted: "Get Started",

      // Dashboard
      dashboardWelcome: "Your farm is looking great today. Here's your daily insight and tools to manage your crops better.",
      smartFarmingTools: "Smart Farming Tools",
      currentWeather: "Current Weather Conditions",
      temperature: "Temperature",
      humidity: "Humidity",
      windSpeed: "Wind Speed",
      rainChance: "Rain Chance",
      recentActivity: "Recent Activity",
      recentActivityDesc: "Your recent farming updates and alerts",

      // Features
      cropRecommendation: "Crop Recommendation",
      cropRecommendationDesc: "Get precise suggestions for the best crops to plant based on your soil health.",
      fertilizerGuide: "Fertilizer Guide",
      fertilizerGuideDesc: "Find the perfect fertilizer mix to boost your yield and maintain soil quality.",
      weatherForecast: "Weather Forecast",
      weatherForecastDesc: "Detailed 7-day weather forecast to help you plan your farming activities.",

      // Crop Recommendation Page
      backToDashboard: "Back to Dashboard",
      cropRecommendationTitle: "Crop Recommendation",
      cropRecommendationSubtitle: "Enter your soil and weather details below. We'll analyze the data and suggest the best crop for your farm.",
      soilNutrients: "Soil Nutrients",
      environment: "Environment",
      nitrogen: "Nitrogen (N)",
      phosphorus: "Phosphorus (P)",
      potassium: "Potassium (K)",
      phLevel: "pH Level",
      rainfall: "Rainfall (mm)",
      getCropRecommendation: "Get Crop Recommendation",
      analyzingSoilData: "Analyzing Soil Data...",
      analysisResult: "Analysis Result",
      bestCropToPlant: "Best Crop to Plant",
      readyToAnalyze: "Ready to Analyze",
      fillFormMessage: "Fill out the form to get a precise recommendation.",
      predictionBased: "Based on your soil's N-P-K values and current weather conditions,",
      predictedToGive: "is predicted to give the highest yield.",

      // Auth
      email: "Email",
      password: "Password",
      fullName: "Full Name",
      confirmPassword: "Confirm Password",
      role: "Role",
      farmer: "Farmer",
      admin: "Admin",
      forgotPassword: "Forgot Password?",
      dontHaveAccount: "Don't have an account?",
      alreadyHaveAccount: "Already have an account?",

      // Footer
      footerTagline: "Nurturing crops with the power of technology.",
      privacyPolicy: "Privacy Policy",
      termsOfService: "Terms of Service",
      contact: "Contact",
      allRightsReserved: "All rights reserved.",

      // Home Page
      heroTitle: "Grow Smarter With",
      heroSubtitle: "Modern Farming",
      heroDescription: "FarmAssist combines biology, data and precision to help farmers make better decisions — from crop selection to fertilizer guidance.",
      keyFeatures: "Key Features",

      // Feature Cards (Home)
      cropRecFeature: "🌾 Crop Recommendation",
      cropRecFeatureDesc: "Get suggestions for the best crops based on soil, weather and region.",
      fertilizerFeature: "🧪 Fertilizer Guide",
      fertilizerFeatureDesc: "Receive fertilizer mixes based on nutrient needs and crop type.",
      weatherFeature: "☁️ Live Weather Alerts",
      weatherFeatureDesc: "Be notified about rainfall, humidity, temperature instantly.",

      // Crop Recommendation Result
      analysisResult: "Analysis Result",
      bestCropToPlant: "Best Crop to Plant",
      predictionMessage: "Based on your soil's N-P-K values and current weather conditions,",
      highestYield: "is predicted to give the highest yield.",
      readyToAnalyze: "Ready to Analyze",
      fillFormMessage: "Fill out the form to get a precise recommendation.",

      // Crop Names
      crop_rice: "Rice",
      crop_maize: "Maize",
      crop_chickpea: "Chickpea",
      crop_kidneybeans: "Kidney Beans",
      crop_pigeonpeas: "Pigeon Peas",
      crop_mothbeans: "Moth Beans",
      crop_mungbean: "Mung Bean",
      crop_blackgram: "Black Gram",
      crop_lentil: "Lentil",
      crop_pomegranate: "Pomegranate",
      crop_banana: "Banana",
      crop_mango: "Mango",
      crop_grapes: "Grapes",
      crop_watermelon: "Watermelon",
      crop_muskmelon: "Muskmelon",
      crop_apple: "Apple",
      crop_orange: "Orange",
      crop_papaya: "Papaya",
      crop_coconut: "Coconut",
      crop_cotton: "Cotton",
      crop_jute: "Jute",
      crop_coffee: "Coffee",

      // Fertilizer Page
      fertilizerTitle: "Fertilizer Recommendation",
      fertilizerSubtitle: "Enter your soil nutrients and select your crop to get the best fertilizer advice.",
      selectCrop: "Select Crop",
      getFertilizerAdvice: "Get Fertilizer Advice",
      analyzingSoil: "Analyzing Soil...",
      recommendationResult: "Recommendation Result",
      recommendedFertilizer: "Recommended Fertilizer",
      expertAdvice: "Expert Advice",

      // Weather Page
      weatherTitle: "Weather Forecast",
      searchCityPlaceholder: "Search your city...",
      search: "Search",
      fetchingWeather: "Fetching Live Weather Data...",
      visibility: "Visibility",
      farmingAdvice: "Farming Advice",
      sevenDayForecast: "7-Day Forecast",
      viewDetailedLog: "View Detailed Log",
      highHumidityAdvice: "High humidity detected. Watch out for fungal diseases in paddy and millets.",
      optimalConditionsAdvice: "Optimal conditions for irrigation and pesticide application today.",
      tempAdvice: "Temp is {temp}°C. Perfect for sowing wheat if moisture levels are adequate.",
};

export default en;
